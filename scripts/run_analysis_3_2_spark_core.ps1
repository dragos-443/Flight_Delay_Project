...
function Save-TimingRow {
    ...
    if ($TimingPath -like "/outputs/benchmarks/scalability/*") {
        $localDir = Join-Path "outputs" "benchmarks\scalability\analysis_3_2\spark_core"
    } else {
        $localDir = Join-Path "outputs" "benchmarks\analysis_3_2\spark_core"
    }
    ...
    $containerTimingPath = "/tmp/analysis_3_2_spark_core_timings.csv"
...
if (-not $OutputRoot) {
    if ($RunSize -eq "scale_all" -or (Test-ScaleRunSize $RunSize)) {
        $OutputRoot = "$hdfsNameNode/outputs/scalability/analysis_3_2/spark_core"
    } else {
        $OutputRoot = "$hdfsNameNode/outputs/analysis_3_2/spark_core"
    }
}

if (-not $BenchmarkRoot) {
    if ($RunSize -eq "scale_all" -or (Test-ScaleRunSize $RunSize)) {
        $BenchmarkRoot = "/outputs/benchmarks/scalability/analysis_3_2/spark_core"
    } else {
        $BenchmarkRoot = "/outputs/benchmarks/analysis_3_2/spark_core"
    }
}
...
if ($RunSize -eq "all") {
    ...
    $localTimingPath = Join-Path "outputs" "benchmarks\analysis_3_2\spark_core\timings.csv"
...
if ($RunSize -eq "scale_all") {
    $runSizes = @("1x", "2x", "4x")
    $localTimingPath = Join-Path "outputs" "benchmarks\scalability\analysis_3_2\spark_core\timings.csv"
    Remove-Item -LiteralPath $localTimingPath -ErrorAction SilentlyContinue
    Invoke-DockerChecked @(
        "compose", "--env-file", ".env",
        "exec", "-T", "namenode",
...
foreach ($currentRunSize in $runSizes) {
    ...
    Write-Host "Eseguo analisi 3.2 Spark Core ($currentRunSize)"
    ...
    $sparkOutput = Invoke-DockerChecked @(
        ...
        "/opt/project/src/analysis_3_2_spark_core.py",
    ...
    if ($CompareWithSparkSql) {
        if (Test-ScaleRunSize $currentRunSize) {
            $sparkSqlParquetPath = "$hdfsNameNode/outputs/scalability/analysis_3_2/spark_sql/$currentRunSize/parquet"
        } else {
        ...
        Invoke-DockerChecked @(
            ...
            "--left", $parquetOutputPath,
            "--right", $sparkSqlParquetPath,
            "--left-label", "spark_core",
