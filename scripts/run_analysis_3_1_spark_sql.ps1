...
function Save-TimingRow {
    ...
    if ($TimingPath -like "/outputs/benchmarks/scalability/*") {
        $localDir = Join-Path "outputs" "benchmarks\scalability\analysis_3_1\spark_sql"
    } else {
        $localDir = Join-Path "outputs" "benchmarks\analysis_3_1\spark_sql"
    }
    ...
    $containerTimingPath = "/tmp/analysis_3_1_spark_sql_timings.csv"
...
if (-not $OutputRoot) {
    if ($RunSize -eq "scale_all" -or (Test-ScaleRunSize $RunSize)) {
        $OutputRoot = "$hdfsNameNode/outputs/scalability/analysis_3_1/spark_sql"
    } else {
        $OutputRoot = "$hdfsNameNode/outputs/analysis_3_1/spark_sql"
    }
}

if (-not $BenchmarkRoot) {
    if ($RunSize -eq "scale_all" -or (Test-ScaleRunSize $RunSize)) {
        $BenchmarkRoot = "/outputs/benchmarks/scalability/analysis_3_1/spark_sql"
    } else {
        $BenchmarkRoot = "/outputs/benchmarks/analysis_3_1/spark_sql"
    }
}
...
if ($RunSize -eq "all") {
    ...
    $localTimingPath = Join-Path "outputs" "benchmarks\analysis_3_1\spark_sql\timings.csv"
...
if ($RunSize -eq "scale_all") {
    $runSizes = @("1x", "2x", "4x")
    $localTimingPath = Join-Path "outputs" "benchmarks\scalability\analysis_3_1\spark_sql\timings.csv"
    Remove-Item -LiteralPath $localTimingPath -ErrorAction SilentlyContinue
    Invoke-DockerChecked @(
        "compose", "--env-file", ".env",
        "exec", "-T", "namenode",
...
foreach ($currentRunSize in $runSizes) {
    ...
    Write-Host "Eseguo analisi 3.1 Spark SQL ($currentRunSize)"
    ...
    $sparkOutput = Invoke-DockerChecked @(
        ...
        "/opt/project/src/analysis_3_1_spark_sql.py",
