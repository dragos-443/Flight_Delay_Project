...
function Save-TimingRow {
    ...
    if ($TimingPath -like "/outputs/benchmarks/scalability/*") {
        $localDir = Join-Path "outputs" "benchmarks\scalability\analysis_3_1\hive"
    } else {
...
if (-not $OutputRoot) {
    if ($RunSize -eq "scale_all" -or (Test-ScaleRunSize $RunSize)) {
        $OutputRoot = "/outputs/scalability/analysis_3_1/hive"
    } else {
...
if (-not $BenchmarkRoot) {
    if ($RunSize -eq "scale_all" -or (Test-ScaleRunSize $RunSize)) {
        $BenchmarkRoot = "/outputs/benchmarks/scalability/analysis_3_1/hive"
    } else {
...
if ($RunSize -eq "scale_all") {
    $runSizes = @("1x", "2x", "4x")
    $localTimingPath = Join-Path "outputs" "benchmarks\scalability\analysis_3_1\hive\timings.csv"
    Remove-Item -LiteralPath $localTimingPath -ErrorAction SilentlyContinue
    Invoke-DockerChecked @(
        "compose", "--env-file", ".env",
        "exec", "-T", "namenode",
